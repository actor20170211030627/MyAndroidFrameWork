...
public class SelectImageVideoAdapter extends BaseQuickAdapter<Object, BaseViewHolder> {
    ...
    private boolean isPictureType = true;
    ...
    protected void convert(@NonNull BaseViewHolder helper, Object item) {
        ImageView iv = helper.addOnClickListener(R.id.iv, R.id.iv_delete)
                .getView(R.id.iv);
        if (item instanceof AlbumFile) {
            Glide.with(mContext).load(((AlbumFile) item).getPath()).into(iv);
        } else if (item instanceof LocalMedia) {
            Glide.with(mContext).load(((LocalMedia) item).getPath()).into(iv);
        }
    }
    ...
    private void preview() {
        if (isPictureType) {
            ...
            PictureSelectorUtils.previewImageVideos((Activity) mContext, true, 0, pics);
        } else {
            ...
            AlbumUtils.galleryAlbum(mContext, albums, 0, false, null);
        }
    }
}
