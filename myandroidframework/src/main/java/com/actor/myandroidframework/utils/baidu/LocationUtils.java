...
public class LocationUtils {
    ...
    protected        LocationClientOption defaultOption, DIYoption;//默认定位配置/自定义定位配置
    ...
    public LocationClient getLocationClient() {
        synchronized (objLock) {
            if (locationClient == null) {
                ...
                /**
                 * fixed 添加了一行, 否则第2次调用定位时没反应,见Demo
                 */
    ...
    public boolean setLocationOption(LocationClientOption option) {
        ...
        if (option != null) {
            if (getLocationClient().isStarted()) getLocationClient().stop();
            DIYoption = option;
     ...
     */
    public @Nullable LocationClientOption getOption() {
        return DIYoption;
    }
